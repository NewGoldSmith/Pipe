...
void PipeToNet::CommandOp::ParseParam(char* p_command, bool flag, bool last)
{
	...
	if (flag)
	{
		...
		//	m_NoOp = false;
		//}
		if (!(_stricmp(p_command, "TimeOut")) || !(_stricmp(p_command, "TO")))
		{
			m_state = option_state::em_time_out;
			m_NoOp = false;
		}
...
bool PipeToNet::CommandOp::IsArgValid()
{
	bool b = true;
	//if (m_Cli == m_Sev)
	//{
