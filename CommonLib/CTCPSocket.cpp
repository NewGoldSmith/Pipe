...
CTCPCliSocket::CTCPCliSocket():CBStream()
{
...
CTCPCliSocket::~CTCPCliSocket()
{
}

bool CTCPCliSocket::Create()
{
...
bool CTCPCliSocket::Bind(std::string strHostAddr, UINT uiHostPort)
{
	//ホストバインド設定
...
bool CTCPCliSocket::Connect( std::string strPeerAddress, USHORT uPeerPort,long lTimeOut=2)
{
...
int CTCPCliSocket::Read(CBinaryString& BD)
{
	int Err;
...
int CTCPCliSocket::Write(const CBinaryString& BD)
{
	int Err = 0;
...
int CTCPCliSocket::GetError()
{
	return m_Err;
}

void CTCPCliSocket::GetSocketStatus(SOCKET Socket, std::string& strResult)
{
...
bool CTCPCliSocket::Wait(long ltimeout)
{
	int Err = 0;
	fd_set rmask = {}, wmask = {};
	FD_ZERO(&rmask);
