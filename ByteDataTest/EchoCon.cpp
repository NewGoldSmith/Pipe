...
int main()
{
    ...
    hr = SetConsoleMode(hStdIn, consoleMode | ENABLE_VIRTUAL_TERMINAL_INPUT)
        ? S_OK
        : GetLastError();
    if (!(S_OK == hr))
    {
   ...
   GetConsoleMode(hStdOut, &consoleMode);
    hr = SetConsoleMode(hStdOut, consoleMode | /*ENABLE_VIRTUAL_TERMINAL_PROCESSING |*/ ENABLE_WRAP_AT_EOL_OUTPUT /* | DISABLE_NEWLINE_AUTO_RETURN*/)
        ? S_OK
        : GetLastError();
    if (S_OK == hr)
    {
        ...
        hToPseudoPipe.hRead = hPipeIn;
        hToPseudoPipe.hWrite = hPipeOut;
        if (S_OK == hr)
        {
            // Create & start thread to listen to the incoming pipe
            // Note: Using CRT-safe _beginthread() rather than CreateThread()
            HANDLE hPipeListenerThread{ reinterpret_cast<HANDLE>(_beginthread(PipeListener, 0, &hToPseudoPipe)) };
...
{
    stHANDLES* pToPseudo = (stHANDLES * )pipe;
    HANDLE hPipe{ pipe };
    HANDLE hStdOut{ GetStdHandle(STD_OUTPUT_HANDLE) };
    HANDLE hStdIn{ GetStdHandle(STD_INPUT_HANDLE) };
    ...
    {
        // Read from the pipe
        fRead = ReadFile(pToPseudo->hRead, szBuffer, BUFF_SIZE, &dwBytesRead, NULL);
        ...
        WriteFile(hStdOut, szBuffer, dwBytesRead, &dwBytesWritten, NULL);
        FlushFileBuffers(hStdOut);
        fRead = ReadFile(hStdIn, szBuffer,  BUFF_SIZE, &dwBytesRead, NULL);
        WriteFile(pToPseudo->hWrite, szBuffer, dwBytesRead, &dwBytesWritten, NULL);
